...
class Site extends CI_Controller {
    ...
    public function editcampaignsubmitbyuser() {
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            if ($this->campaignaccess_model->edit($id, $Name, $emailused, $question, $deadline, $goals, $audience, $callactivate, $reqelem, $keywords, $specific, $outline, $avoid, $material, $instructions, $uorganization, $industryp, $propositions, $propositions1, $propositions2, $itrends, $mcompetitors, $branding, $specificfile, $outlinefile, $materialfile, $brandingfile, $user) == 0) $data["alerterror"] = "New campaign could not be Updated.";
            else $data["alertsuccess"] = "campaign Updated Successfully.";
            $data["redirect"] = "site/viewmycampaign";
            $this->load->view("redirect", $data);
        }
    }
    ...
    public function createcampaignsubmit() {
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            if ($campaignid == 0) $data["alerterror"] = "New Campaign could not be created.";
            else {
    ...
    public function createcampaigngroupsubmit() {
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            if ($this->campaigngroup_model->create($campaign, $order, $status, $group) == 0) $data["alerterror"] = "New Campaign Group could not be created.";
            else $data["alertsuccess"] = "Campaign Group created Successfully.";
